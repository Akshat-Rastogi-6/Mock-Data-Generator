...
from ingest_transform import store_model_path as smpsql  # Function to store model path in PostgreSQL
from ingest_transform_couchdb import store_model_path as smpcdb  # Function to store model path in CouchDB
...
def train_(df, epochs, database_choice):
    ...
    # Store paths of saved models in the selected database (PostgreSQL or CouchDB)
    if database_choice == "PostgreSQL":
        smpsql(os.path.join(model_dir, 'generator_model.h5'),
               os.path.join(model_dir, 'discriminator_model.h5'),
               os.path.join(model_dir, 'gan_model.h5'))
    else:
        smpcdb(os.path.join(model_dir, 'generator_model.h5'),
               os.path.join(model_dir, 'discriminator_model.h5'),
               os.path.join(model_dir, 'gan_model.h5'))
